...
public:
  vector<int> findRedundantConnection( vector<vector<int>>& edges ) {
    ...
    vector<int> lastRedunEdge;
    for( auto edge : edges ) {
      int root1 = find( groups, edge[0] - 1 );
      int root2 = find( groups, edge[1] - 1 );
      if( root1 != root2 ) groups[root1] = root2; // Union
      else lastRedunEdge = edge;
    }
    return lastRedunEdge;
  }
};
